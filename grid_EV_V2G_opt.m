function [Optimized_House_grid_EV_V2G,grid_EV_V2G,All_Power_grid_EV_V2G]=grid_EV_V2G_opt(All_Power_grid_EV_nonV2G,bigdata_Houses,excelPowerbyTime,initial_time,desired_time,p_other,ev_capacity,initial_SOC,desired_SOC,buy_price,sell_price,NumberofElectricVehicle)
...
sebekenonv2g=sum(All_Power_grid_EV_nonV2G);
sebekemax=max(sebekenonv2g)-2;

const=[];
const=[const,max_power_limit==sum(buy,2)];
const=[const,max_power_limit<=sebekemax];

for i=1:NumberofElectricVehicle
    ev_Charge_max(i,1)=max(excelPowerbyTime(i,:))*1.2;
...
const=[const,const,sell(t,i) ==ev_bat_sold(t,i)];
const=[const,ev_bat_ch_grid(t,i) == ev_bat_ch(t,i)*eff];
const=[const,grid(t,i)+ev_bat_used(t,i)==p_other(t,i)];
const=[const,ev_bat_sold(t,i)+ev_bat_used(t,i) == ev_bat_disch(t,i)*eff];
const=[const,ev_bat(t,i)<=desired_SOC(1,i)];
const=[const,0 <= [grid(t,i)  sell(t,i) buy(t,i) midsum(t,i) ev_bat_ch(t,i) ev_bat_disch(t,i) ev_bat_ch_grid(t,i)  ev_bat_sold(t,i) ev_bat_used(t,i)] <= 1000];
    end
end

for i=1:NumberofElectricVehicle
const=[const,ev_bat(1:initial_time(1,i),i)==initial_SOC(1,i)]; % arac?n ba?lang?ç yüzdesi ile de?i?cek bu sayi
...
total_cost_grid_EV_V2G=midsum;
%optimization
options = sdpsettings('solver','cplex');
sol = solvesdp(const,total_cost_grid_EV_V2G,options);
% sol = solvesdp(const,deneme,options);

for i=1:NumberofElectricVehicle
D = table([value(p_other(:,i))],[value(grid(:,i))],[value(sell(:,i))],[value(ev_bat_disch(:,i))],[value(ev_bat_ch_grid(:,i))],[value(ev_bat(:,i))],[value(buy(:,i))],...
    'VariableNames',{ 'p_other' 'grid' 'sell' 'ev_bat_disch' 'ev_bat_ch_grid' 'ev_bat' 'buy'},...
...
total_cost_grid_EV_V2G = table([value(midsum(1,i))],...
                 'VariableNames',{'total_cost_V2G'},...
                 'RowNames',{'baking'});
   
    d0=D;
    Optimized_House_grid_EV_V2G{i,1}=d0;
    grid_EV_V2G(i,1)=total_cost_grid_EV_V2G;
    All_Power_grid_EV_V2G(i,:)=value(buy(:,i)');
end
if sol.problem == 0
solution = value(total_cost_grid_EV_V2G);
else
